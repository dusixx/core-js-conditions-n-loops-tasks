...
function convertNumberToString(str) {
  ...
  for (let i = 0, splitter = ' '; i < str.length; i += 1) {
    const word = map[str[i]];
...
function sortByAsc(arr) {
  function qsort(left = 0, right = arr.length - 1) {
    const a = arr;
    const pivot = a[Math.trunc((right + left) / 2)];
    let i = left;
    let j = right;
...
function shuffleChar(str, iterations, useCache = true) {
  ...
  const shuffled = { 0: str };
  const me = shuffleChar;

  me.fullCycle = me.fullCycle || {};
...
function getNearestBigger(number) {
  ...
  l.length -= r.length;
  for (let i = 0, k = l.length - 1; i < r.length; i += 1) {
    if (r[i] > l[k]) {
      [r[i], l[k]] = [l[k], r[i]];
      break;
    }
  }
  return Number([...l, ...r].join(''));
}
